  ...
  var password = {
    ...
    generate: function (pwdLength, options) {
      ...
      if (this.validated(password, options)) {
        ...
        this.generate(pwdLength, options);
      }
    },
...
function usee() {
  let url =
    "https://velyrian.github.io/main/ai/password-generator/usernames/colors.json";

  fetch(url)
    .then((res) => res.json())
    .then((out) => username(out))
    .catch((err) => {
      throw err;
    });
}
...
function username(data) {
  var all = "";
  var user = "";
  var inputadv = document.getElementById("customAdv");
  var inputVerb = document.getElementById("customNoun");
  count = 0;

  var adv = data.adv[Math.floor(Math.random() * data.adv.length)];
  var noun = data.noun[Math.floor(Math.random() * data.noun.length)];
  if (inputadv && inputadv.value) {
    adv = inputadv.value;
  }

  if (inputVerb && inputVerb.value) {
    noun = inputVerb.value;
  }
  all = adv + noun;
  document.getElementById("generated-username").innerHTML =
    all + Math.floor(Math.random() * 999);
}
